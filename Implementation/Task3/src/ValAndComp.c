...
int parenthesesVal(const char express[])
{
    ...
    int openParCount=0; //stores the count of open parentheses

    /*Count parentheses for use in malloc*/
    for(index=0; index < expressLength; index++)
        if(express[index] == '(')
            openParCount++;

    /*Declaring a pointer to a stack*/
    struct stack *parenStack = malloc(sizeof(struct stack)
                             + openParCount*sizeof(float));
