...
void infixToPostfix(const char infixExp[], char postfixExp[])
{
    ...
    int operatorCount=0; //stores the count of operators

    /*Counting the operators in the expression*/
    for(infxIndex=0; infxIndex < expressLength; infxIndex++)
        if(isOperator(infixExp[infxIndex]))
            operatorCount++;

    /*Declaring a pointer to a stack*/
    struct stack *opStack = malloc(sizeof(struct stack)
                          + operatorCount*sizeof(float));
...
float evaluateExpress(char postfixExp[])
{
    ...
    int operatorCount;        //stores the count of operators

    /*Counting the operators in the expression*/
    for(index=0; index < strlen(postfixExp); index++)
        if(isOperator(postfixExp[index]))
            operatorCount++;

    /*Declaring a pointer to a stack*/
    struct stack *postfixStack = malloc(sizeof(struct stack)
                               + (1+operatorCount)*sizeof(float));
